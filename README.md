...
- 每次 B30 都直接读取落雪最新的玩家资料快照，不使用插件侧玩家缓存；当玩家资料的同步时间早于成绩时，图片会标记 `PROFILE STALE` 并提醒重新同步。
- 新增 `/chu sync`（别名 `/chu 同步`、`/chu 传分`），返回落雪官方同步说明与中二节奏同步入口。

## v0.6.3 修复

- B30 左上角 Rating 改为根据本次 `Best 30 + New 20` 的固定 50 个槽位重算，并按游戏显示规则截断至两位小数；当落雪玩家资料仍是更新前快照时，不再显示旧 Rating。

## v0.6.2 修复

- B30 明确区分“历代高分”和“2027 已游玩新曲”；`NEW 20` 仅显示已有成绩的当期谱面，未游玩的新谱不会以空卡片补齐。

## v0.6.1 修复

- B30 遇到没有匹配到定数的谱面时，会立刻强制刷新一次曲库并重新匹配，避免游戏更新日复用旧缓存导致新曲显示 `-`。
- 曲库默认缓存时间缩短为 1 小时，降低新曲、改定数的等待时间。
...
| `cache_seconds` | `3600` | 曲库和别名本地缓存时间，单位秒；B30 遇到未匹配到定数的新谱面会立即刷新一次曲库。 |
